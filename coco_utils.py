...
@ray.remote
def compute_coco_group(data, num_players):
    # Tell GLPK to produce no output, and timeout after 1 second
    solvers.options['glpk'] = {'msg_lev': 'GLP_MSG_OFF', 'tm_lim': 1000}
...
def compute_coco_distributed(data, num_actors, num_players):
    data_groups = np.array_split(data, num_actors)
    answer_refs = [compute_coco_group.remote(data_group, num_players) for data_group in data_groups]
